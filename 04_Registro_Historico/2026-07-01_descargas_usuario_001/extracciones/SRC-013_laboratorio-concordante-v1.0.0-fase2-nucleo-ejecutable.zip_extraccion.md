# SRC-013 - extraccion textual

Fuente: `laboratorio-concordante-v1.0.0-fase2-nucleo-ejecutable.zip`

Estatus: material historico extraido automaticamente; no autoridad directa.

## Texto

## laboratorio-concordante-v1.0.0-fase2/README.md
...
## laboratorio-concordante-v1.0.0-fase2/INDEX.md
...
## laboratorio-concordante-v1.0.0-fase2/BASELINE.md
...
## laboratorio-concordante-v1.0.0-fase2/CHANGELOG.md
...
- Núcleo ejecutable AAU v0.1 reforzado.
- Paquete importable con `__init__.py`.
- Tests mínimos ampliados.
- Configuración `pyproject.toml` para pytest.


## laboratorio-concordante-v1.0.0-fase2/pyproject.toml

[tool.pytest.ini_options]
pythonpath = ["."]
testpaths = ["IMPLEMENTACION/tests"]


## laboratorio-concordante-v1.0.0-fase2/CANON/C-000_Principio_de_Alcance.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/M-000_Auditoria_Interna.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/M-001_Presuncion_de_Reducibilidad.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/C-001_Reduccion_Conceptual.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/C-002_Criterio_de_Parada.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/EF-001_Economia_Formal.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/Clausula_de_Incompletitud.md
...
## laboratorio-concordante-v1.0.0-fase2/CANON/COMPATIBILIDAD.md
...
## laboratorio-concordante-v1.0.0-fase2/MATEMATICA/Notacion.md
...
## laboratorio-concordante-v1.0.0-fase2/MATEMATICA/Definiciones.md
...
## laboratorio-concordante-v1.0.0-fase2/MATEMATICA/Teoremas.md
...
## laboratorio-concordante-v1.0.0-fase2/MATEMATICA/Demostraciones.md
...
## laboratorio-concordante-v1.0.0-fase2/MATEMATICA/Problemas_Abiertos.md
...
## laboratorio-concordante-v1.0.0-fase2/ARQUITECTURA/Firmas_Estructurales.md
...
## laboratorio-concordante-v1.0.0-fase2/ARQUITECTURA/Invariantes.md
...
## laboratorio-concordante-v1.0.0-fase2/ARQUITECTURA/Grafo_Acoplamiento_Invariantes.md
...
## laboratorio-concordante-v1.0.0-fase2/ARQUITECTURA/R4.md
...
## laboratorio-concordante-v1.0.0-fase2/ARQUITECTURA/AAU.md
...
## laboratorio-concordante-v1.0.0-fase2/ARQUITECTURA/SE-001.md
...
## laboratorio-concordante-v1.0.0-fase2/PROTOCOLOS/Auditoria.md
...
## laboratorio-concordante-v1.0.0-fase2/PROTOCOLOS/Admision.md
...
## laboratorio-concordante-v1.0.0-fase2/PROTOCOLOS/Reduccion.md
...
## laboratorio-concordante-v1.0.0-fase2/PROTOCOLOS/Validacion.md
...
## laboratorio-concordante-v1.0.0-fase2/PROTOCOLOS/Demolicion.md
...
## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/auditor_laboratorio.py
...
from typing import FrozenSet, Set, Dict, Iterable
...
class Invariante:
    """
    Firma preservable evaluable por el Auditor.

    id:
        Identificador estable de la firma.
    is_computable:
        Indica si la firma es decidible/auditable.
    is_satisfied:
        Indica si G satisface la firma: G ⊨ F.
    embedding_nodes / embedding_edges:
        Soporte estructural usado para construir G_I por acoplamiento.
    """

    id: str
    is_computable: bool = True
    is_satisfied: bool = True
    embedding_nodes: FrozenSet[str] = field(default_factory=frozenset)
    embedding_edges: FrozenSet[str] = field(default_factory=frozenset)


@dataclass(frozen=True)
class AuditoriaSalida:
    estado: str
    componente_ruptura: FrozenSet[str]
    justificacion: str
    firmas_no_decidibles: FrozenSet[str] = field(default_factory=frozenset)
    conflicto: FrozenSet[str] = field(default_factory=frozenset)


class AuditorLaboratorio:
    """
    Implementación de referencia del AAU v0.1.

    Entrada:
        presv: conjunto de firmas preservables.
        afirmaciones: Activas(G), firmas que el modelo mantiene como vigentes.

    Salida:
        AuditoriaSalida con estado y justificación mínima.
    """

    def __init__(self, presv: Iterable[Invariante], afirmaciones: Iterable[str] | None = None):
        self.presv: Set[Invariante] = set(presv)
        self.activas: Set[str] = set(afirmaciones or [])
        self.grafo_gi: Dict[str, Set[str]] = {}

    def construir_gi(self, inv_satisfechos: Set[Invariante]) -> None:
        """
        Construye G_I como grafo no dirigido de acoplamiento entre invariantes.

        Dos invariantes se conectan si sus embeddings comparten al menos
        ...
        for i, fa in enumerate(invariantes):
            for fb in invariantes[i + 1:]:
                if self._estan_acoplados(fa, fb):
                    self.grafo_gi[fa.id].add(fb.id)
                    self.grafo_gi[fb.id].add(fa.id)

    @staticmethod
    def _estan_acoplados(fa: Invariante, fb: Invariante) -> bool:
        return bool(fa.embedding_nodes & fb.embedding_nodes) or bool(
            fa.embedding_edges & fb.embedding_edges
        )
    ...
    def ejecutar_aau(self) -> AuditoriaSalida:
        ...
        if not self.presv:
            return AuditoriaSalida(
                estado=ESTADO_NO_AUDITABLE,
                componente_ruptura=frozenset(),
                justificacion="Presv no definido o vacío.",
            )

        no_decidibles = frozenset(f.id for f in self.presv if not f.is_computable)
        if no_decidibles:
            return AuditoriaSalida(
                estado=ESTADO_NO_AUDITABLE,
                componente_ruptura=frozenset(),
                justificacion=f"Firmas preservables no decidibles: {sorted(no_decidibles)}.",
                firmas_no_decidibles=no_decidibles,
            )
        ...
        if not inv_g:
            return AuditoriaSalida(
                estado=ESTADO_NO_AUDITABLE,
                componente_ruptura=frozenset(),
                justificacion="No hay invariantes satisfechos auditables.",
            )
        ...
        if not rupturas:
            return AuditoriaSalida(
                ...
                componente_ruptura=frozenset(),
                justificacion="Sin rupturas detectadas.",
            )
        ...
        for f in rupturas:
            if f.id in self.grafo_gi:
                ...
                # Firma rota que no pertenece a Inv(G_new). Se reporta como ruptura local.
                break_fk.add(f.id)

        conflicto = frozenset(self.activas & break_fk)

        if conflicto:
            return AuditoriaSalida(
                estado=ESTADO_INCONSISTENTE,
                componente_ruptura=frozenset(break_fk),
                justificacion=f"Activas(G) intersecta Break(Fk): {sorted(conflicto)}.",
                conflicto=conflicto,
            )
        ...
        return AuditoriaSalida(
            ...
            componente_ruptura=frozenset(break_fk),
            justificacion="Zona de riesgo estructural detectada sin contradicción declarativa.",
        )
...
## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/pseudocodigo.md
...
## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/__init__.py



## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/README.md

# Implementación de referencia — AAU v0.1
...
- `auditor_laboratorio.py`: núcleo ejecutable del AAU.
- `tests/test_aau.py`: suite mínima de pruebas.


## laboratorio-concordante-v1.0.0-fase2/EXPEDIENTES/P-110_Asimetria_Explicativa.md
...
## laboratorio-concordante-v1.0.0-fase2/EXPEDIENTES/P-111_Auditoria_del_Modelo.md
...
## laboratorio-concordante-v1.0.0-fase2/STRESS_TESTS/Horoscopo.md
...
## laboratorio-concordante-v1.0.0-fase2/STRESS_TESTS/Bacteria.md
...
## laboratorio-concordante-v1.0.0-fase2/STRESS_TESTS/Empresa.md
...
## laboratorio-concordante-v1.0.0-fase2/STRESS_TESTS/Red_Neuronal.md
...
## laboratorio-concordante-v1.0.0-fase2/STRESS_TESTS/Delirio.md
...
## laboratorio-concordante-v1.0.0-fase2/STRESS_TESTS/Ajedrez.md
...
## laboratorio-concordante-v1.0.0-fase2/HISTORIA/H-000_Nacimiento_del_Programa.md
...
## laboratorio-concordante-v1.0.0-fase2/HIPOTESIS/HIPOTESIS_ACTIVAS.md
...
## laboratorio-concordante-v1.0.0-fase2/GLOSARIO/Glosario.md
...
## laboratorio-concordante-v1.0.0-fase2/.pytest_cache/README.md
...
## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/tests/test_aau.py

from IMPLEMENTACION.auditor_laboratorio import (
    Invariante,
    AuditorLaboratorio,
    ESTADO_APROBADO,
    ESTADO_SUSPENDIDO,
...
def inv(id, computable=True, satisfied=True, nodes=(), edges=()):
    return Invariante(
        ...
        embedding_nodes=frozenset(nodes),
        embedding_edges=frozenset(edges),
    )


def test_no_auditable_por_presv_vacio():
    salida = AuditorLaboratorio(set(), set()).ejecutar_aau()
    assert salida.estado == ESTADO_NO_AUDITABLE


def test_no_auditable_por_firma_no_decidible():
    f1 = inv("F1", computable=False, nodes=("A",))
    salida = AuditorLaboratorio({f1}, set()).ejecutar_aau()
    assert salida.estado == ESTADO_NO_AUDITABLE
    assert salida.firmas_no_decidibles == frozenset({"F1"})


def test_no_auditable_sin_invariantes_satisfechos():
...
def test_aprobado_sin_rupturas():
    ...
    assert salida.estado == ESTADO_APROBADO
    assert salida.componente_ruptura == frozenset()


def test_suspendido_por_ruptura_sin_conflicto():
    # F1 roto, F2 satisfecho. No hay Activas(G) que niegue la zona de riesgo.
    f1 = inv("F1", satisfied=False, nodes=("A",))
    f2 = inv("F2", satisfied=True, nodes=("A",))
    salida = AuditorLaboratorio({f1, f2}, set()).ejecutar_aau()
    assert salida.estado == ESTADO_SUSPENDIDO
    assert "F1" in salida.componente_ruptura


def test_inconsistente_por_activa_en_break():
    f1 = inv("F1", satisfied=False, nodes=("A",))
    f2 = inv("F2", satisfied=True, nodes=("A",))
    salida = AuditorLaboratorio({f1, f2}, {"F1"}).ejecutar_aau()
    assert salida.estado == ESTADO_INCONSISTENTE
    assert salida.conflicto == frozenset({"F1"})


def test_gi_acopla_por_nodo_compartido():
    f1 = inv("F1", nodes=("A", "B"))
    f2 = inv("F2", nodes=("B", "C"))
    auditor = AuditorLaboratorio({f1, f2}, set())
    auditor.construir_gi({f1, f2})
    assert auditor.grafo_gi["F1"] == {"F2"}
    assert auditor.grafo_gi["F2"] == {"F1"}


def test_gi_no_acopla_si_no_comparten_embedding():
    f1 = inv("F1", nodes=("A",))
    f2 = inv("F2", nodes=("B",))
    auditor = AuditorLaboratorio({f1, f2}, set())
    auditor.construir_gi({f1, f2})
    assert auditor.grafo_gi["F1"] == set()
    assert auditor.grafo_gi["F2"] == set()


## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/tests/__init__.py



## laboratorio-concordante-v1.0.0-fase2/IMPLEMENTACION/tests/TEST_OUTPUT.txt
...
rootdir: /mnt/data/laboratorio-concordante-v1.0.0-fase2
configfile: pyproject.toml
testpaths: IMPLEMENTACION/tests
plugins: asyncio-1.3.0, metadata-3.1.1, Faker-40.1.2, cov-7.0.0, ddtrace-4.4.0, anyio-4.13.0, json-report-1.5.0
asyncio: mode=Mode.STRICT, debug=False, asyncio_default_fixture_loop_scope=None, asyncio_default_test_loop_scope=function
collected 8 items

IMPLEMENTACION/tests/test_aau.py [32m.[0m[32m.[0m[32m.[0m[32m.[0m[32m.[0m[32m.[0m[32m.[0m[32m.[0m[32m                                [100%][0m

[32m============================== [32m[1m8 passed[0m[32m in 0.36s[0m[32m ===============================[0m
