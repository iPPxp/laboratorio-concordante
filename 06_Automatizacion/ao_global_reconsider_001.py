#!/usr/bin/env python3
"""Reconsideracion global no mutante posterior a evidencia externa real."""
...
ALGORITHM_ID = "AO-GLOBAL-RECONSIDER-001"
EXPEDIENTE = "AO-001"
DEFAULT_CASE_FILE = Path(__file__).resolve().parent / "fixtures" / "ao_global_reconsider_001_matrix.json"


@dataclass(frozen=True)
class ReconsiderConditionResult:
    condition_id: str
    front: str
    expected: str
    actual: str
    passed: bool
    blocks_global_reconsideration: bool
    local_credit: bool
    gap: str
    evidence: tuple[str, ...]
...
def evaluate_condition(condition: dict[str, Any]) -> ReconsiderConditionResult:
    actual = str(condition.get("status", "no_detectada"))
    expected = str(condition.get("expected", actual))
    return ReconsiderConditionResult(
        condition_id=str(condition.get("condition_id", "")),
        front=str(condition.get("front", "")),
        expected=expected,
        actual=actual,
        passed=actual == expected,
        blocks_global_reconsideration=bool(condition.get("blocks_global_reconsideration")),
...
def summarize(results: list[ReconsiderConditionResult]) -> dict[str, Any]:
    failures = [result for result in results if not result.passed]
    blockers = [result for result in results if result.blocks_global_reconsideration]
...
def build_report(root: Path | None = None, case_file: Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "suite_id": suite.get("suite_id", "AO-GLOBAL-RECONSIDER-001-MATRIX"),
        "resultado": "ok" if not failures else "bloqueado",
        "recomendacion": "mantener_no_autorizado_y_preparar_paquete_validacion_externa",
        "transformacion_permitida": False,
        "global_reconsideration_result": "mantener_no_autorizado_con_perfiles_formales_preparados",
        "ready_for_global_decision": False,
        "external_evidence_ready": True,
        "report_layer_formal_profile_prepared": True,
        "r4_gamma_export_profile_prepared": True,
        ...
        "next_logical_route": "AO-EXT-VALIDATION-PACKET-001",
        ...
        "scope_guard": {
            ...
            "ejecuta_validacion_externa": False,
        },
...
def render_md(report: dict[str, Any]) -> str:
    lines = [
        "# AO_GLOBAL_RECONSIDER_001_REPORT",
        "",
        ...
        "transformacion_permitida: false",
        f"global_reconsideration_result: {report['global_reconsideration_result']}",
        "ready_for_global_decision: false",
        "external_evidence_ready: true",
        "report_layer_formal_profile_prepared: true",
        "r4_gamma_export_profile_prepared: true",
        ...
        "global_equivalence_authorized: false",
        "global_confluence_authorized: false",
        f"next_logical_route: {report['next_logical_route']}",
        "",
        "## Resumen",
        "",
    ]
    ...
    for result in report["condition_results"]:
        ...
        block = "bloquea" if result["blocks_global_reconsideration"] else "no_bloquea"
    ...
    lines.append("- La evidencia externa real permite reconsiderar readiness, pero no basta para autorizar promocion/exportacion general.")
    lines.append("- La siguiente ruta logica es preparar paquete de validacion externa independiente sin ejecutarlo.")
    return "\n".join(lines) + "\n"
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Reconsideracion global AO no mutante.")
