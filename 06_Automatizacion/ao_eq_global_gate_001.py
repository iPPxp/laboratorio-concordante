#!/usr/bin/env python3
"""Compuerta no mutante de Equivalencia global AO-EQ-GLOBAL-GATE-001."""
...
ALGORITHM_ID = "AO-EQ-GLOBAL-GATE-001"
EXPEDIENTE = "AO-001"
DEFAULT_CASE_FILE = Path(__file__).resolve().parent / "fixtures" / "ao_eq_global_gate_001_matrix.json"
...
class GateConditionResult:
    ...
    status: str
    local_progress: bool
    blocks_global_equivalence: bool
    evidence: tuple[str, ...]
    gap: str
    passed: bool
    warnings: tuple[str, ...]
...
def evaluate_condition(condition: dict[str, Any]) -> GateConditionResult:
    warnings: list[str] = []
    ...
    blocks = bool(condition.get("blocks_global_equivalence"))
    ...
    if status in {"faltante_global", "bloqueada_por_alcance"} and not blocks:
        warnings.append("faltante_no_bloquea_equivalencia")
    if bool(condition.get("global_equivalence_claim")):
        warnings.append("afirma_equivalencia_global")
    ...
    return GateConditionResult(
        ...
        blocks_global_equivalence=blocks,
        evidence=evidence,
...
def summarize(results: list[GateConditionResult]) -> dict[str, Any]:
    ...
    return {
        ...
        "global_blocking": sum(1 for result in results if result.blocks_global_equivalence),
        "by_status": by_status,
        "by_domain": by_domain,
    }
...
def build_report(root: Path | None = None, case_file: Path | None = None) -> dict[str, Any]:
    ...
    failures = [result for result in results if not result.passed]
    return {
        "report_id": "AO-EQ-GLOBAL-GATE-001-" + dt.datetime.now().strftime("%Y%m%d-%H%M%S"),
        "expediente": EXPEDIENTE,
        "algoritmo": ALGORITHM_ID,
        "suite_id": suite.get("suite_id", "AO-EQ-GLOBAL-GATE-MATRIX-001"),
        "resultado": "ok" if not failures else "bloqueado",
        "recomendacion": "mantener_equivalencia_global_no_autorizada" if not failures else "revisar_compuerta_equivalencia_global",
...
def render_md(report: dict[str, Any]) -> str:
    ...
    for result in report["condition_results"]:
        ...
        lines.append(f"- {status} `{result['condition_id']}` ({result['domain']}): {result['status']}")
    ...
    if report["findings"]:
        ...
        lines.append("- La equivalencia global permanece no autorizada por faltantes globales.")
    return "\n".join(lines) + "\n"
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Compuerta AO de equivalencia global no mutante.")
